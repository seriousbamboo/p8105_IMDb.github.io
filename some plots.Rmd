...
In this page, we focus on exploring the data. There are many variables in this dataset. Thus, before do any further exploration, firstly let's summary the dataset, which is the best way to help us study this dataset.

After knowing the summary of the datset, we need to explore the association between the variables and find some potential predictors for gross.
...
directors = 
  ...
  labs( x = "Director's Facebook Likes",
        y = "Gross",
        title = "Association between Director's Facebook Likes and Gross")
ggplotly(directors)
```
...
imbdscoreplot = 
  ...
  geom_smooth(method='lm', se=TRUE) +
  labs(x="IMDB score of movie", y="Movie gross", title = "Association between IMBD Score and Gross")
...
totalfacebook = 
  ...
  labs(
    ...
    y = "Gross",
    title = "Association between Cast Total Facebook Likes and Gross"
  )
...
ep_dataset %>% 
  ...
  labs(x = "Budget",
       y = "Gross",
       title = "Association between Budget and Gross")
```
